...
struct quirc {
	...
	/* Копия исходного полутонового кадра. Нужна потому, что при
	 * QUIRC_PIXEL_ALIAS_IMAGE буфер pixels алиасит image, и первая же
	 * бинаризация затирает полутона. Сохранение оригинала позволяет
	 * (а) корректно прогонять несколько проходов бинаризации по реальным
	 * полутонам, (б) считывать модули по полутонам на этапе декодирования. */
	uint8_t			*gray;
	int			w;
	int			h;
